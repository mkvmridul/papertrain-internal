...
5. **Acts**, behind a human. The report names the one action that fits the break type and drafts the message it will carry. On "approve", the matching Elastic Workflow runs: open a case for a missing credit, raise a reversal with the partner for a double debit, log a fee dispute, close a timing break as self-clearing, or escalate an unresolved one. Every action writes an append-only audit record. The agent cannot do any of this on its own.
...
- **Actions are gated.** The only writes the agent can perform are the five workflow tools in `elastic/workflows/actions.json`, one per break type, and only after the user types approve. The workflows themselves compute nothing: every field is passed through.
...
| Agent Builder | Ten tools (five `esql`, five `workflow`), one agent with the instructions in `elastic/agent/instructions.md`, created through the Kibana API by `src/setup-agent.js`. The per-run trace waterfall is the explainability artefact shown on stage. |
| Elastic Workflows + Cases | Five workflows in `elastic/workflows/`, one per break type, each exposed to the agent as a tool: `cases.createCase` where a case is warranted (missing credit, double debit, unresolved), then `elasticsearch.request` to write the audit record. Fee disputes and timing closures write the record without a case. |
...
elastic/workflows/*.yaml        the five action workflows; actions.json maps break type -> tool -> workflow
